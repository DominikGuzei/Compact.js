...
var DOMAssistant = function () {
	...
	return {
		...
		cssByDOM : function (cssRule) {
			...
			function getElementsByPseudo (previousMatch, pseudoModule, pseudoValue) {
				prevParents = [];
				var pseudo = pseudoModule.split("-"), matchingElms = [], idx = 0, checkNodeName = /\-of\-type$/.test(pseudoModule), recur,
				match = {
					...
					other: function(el) { return getAttr(el, pseudoModule) === pseudoValue; }
				};
				...
				switch (word) {
					...
					case "not":
						if ((recur = regex.pseudo.exec(pseudoValue))) {
							...
							var notModule = regex.classes.exec(pseudoValue);
							var notAttr = regex.attribs.exec(pseudoValue);
							var notRegExp = new RegExp(notAttr? attrToRegExp(notAttr[4], notAttr[2]) : "(^|\\s)" + (notTag? notTag[0] : notModule? notModule[1] : "") + "(\\s|$)", "i");
							while ((notElm=previousMatch[idx++])) {
								addElm = null;
								if (notTag && !notRegExp.test(notElm.nodeName) || notModule && !notRegExp.test(notElm.className)) {
									addElm = notElm;
								}
								else if (notAttr) {
			...
			for (var a=0, tagBin=[]; (currentRule=cssRules[a]); a++) {
				...
				for (var i=0, rule; (rule=cssSelectors[i]); i++) {
					...
					if (splitRule.allModulees) {
						var n = 0, matchingModuleElms = [], allModulees = splitRule.allModulees.split(".").slice(1);
						while ((current = prevElm[n++])) {
							var matchCls = true, elmModule = current.className;
							if (elmModule && elmModule.length) {
								elmModule = elmModule.split(" ");
								for (var o=allModulees.length; o--;) {
									if (elmModule.indexOf(allModulees[o]) < 0) {
										matchCls = false;
										break;
									}
								}
								if (matchCls) {
									matchingModuleElms[matchingModuleElms.length] = current;
								}
							}
						}
						prevElm = matchingElms = matchingModuleElms;
					}
					...
					if (splitRule.allPseudos) {
						...
						for (var t=0, tl=allPseudos.length; t<tl; t++) {
							...
							var pseudoModule = pseudo[1]? pseudo[1].toLowerCase() : null;
							var pseudoValue = pseudo[3] || null;
							matchingElms = getElementsByPseudo(matchingElms, pseudoModule, pseudoValue);
		...
		cssByXpath : function (cssRule) {
			...
			DOMAssistant.cssByXpath = function (cssRule) {
				...
				function pseudoToXPath (tag, pseudoModule, pseudoValue) {
					tag = /\-child$/.test(pseudoModule)? "*" : tag;
					var pseudo = pseudoModule.split("-"), position = ((pseudo[1] === "last")? "(count(following-sibling::" : "(count(preceding-sibling::") + tag + ") + 1)", recur, hash;
					switch (pseudo[0]) {
						...
						default: return "@" + pseudoModule + "=\"" + pseudoValue + "\"";
					}
				}
				for (var i=0; (currentRule=cssRules[i]); i++) {
					...
					for (var j=0, jl=cssSelectors.length; j<jl; j++) {
						...
						xPathExpression +=
							...
							(splitRule.allModulees || "").replace(regex.classes, "[contains(concat(\" \", @class, \" \"), \" $1 \")]") +
						...
						if (splitRule.allPseudos) {
							...
							for (var k=0, kl=allPseudos.length; k<kl; k++) {
								...
								var pseudo = regex.pseudos.exec(allPseudos[k]),
									pseudoModule = pseudo[1]? pseudo[1].toLowerCase() : null,
									pseudoValue = pseudo[3] || null,
									xpath = pseudoToXPath(splitRule.tag, pseudoModule, pseudoValue);
								if (xpath.length) {
		...
		elmsByModule : function (className, tag) {
